...
public class AlunoManager {
	...
	private Long recuperaIdCoordenador(String login) {
		...
		return (Long )em.createQuery("select coordenador.id from Usuario usu inner join usu.coordenador as coordenador where usu.login = :login")
	 ...
	 * @return
	 */
	public List<AlunoDisciplinaTurma> buscaNotasAlunosCoordenador(
			String dataInicial, String dataFinal, Disciplina disciplina,
			Aluno aluno, Curso curso, String login) {
		EntityManager em = FactoryBanco.getInstance().getEntityManager();
		Long idCoordenador = recuperaIdCoordenador(login);
		StringBuilder query = new StringBuilder("select distinct obj from AlunoDisciplinaTurma obj left outer join obj.notas notas inner join obj.aluno aluno where obj.disciplinaTurma.curso.coordenador.id = :idCoordenador");
		if (dataInicial!=null && !dataInicial.equals("")){
			query.append(" and notas.data >= :dataInicial");
		}
		if (dataFinal!=null && !dataFinal.equals("")){
	...
	public List<AlunoDisciplinaTurma> buscaNotasAlunosAtendente(
			String dataInicial, String dataFinal, Aluno aluno) {
		EntityManager em = FactoryBanco.getInstance().getEntityManager();
	 
		StringBuilder query = new StringBuilder("select distinct obj from AlunoDisciplinaTurma obj left outer join obj.notas notas inner join obj.aluno aluno where  1 = 1");
		if (dataInicial!=null && !dataInicial.equals("")){
			query.append(" and notas.data >= :dataInicial");
		}
		if (dataFinal!=null && !dataFinal.equals("")){
